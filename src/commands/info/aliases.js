  ...
  run(message, args) {
    ...
    if (args[0] && types.includes(type) && (type != OWNER || message.client.isOwner(message.member))) {
      ...
      embed
        ...
        .setThumbnail('https://raw.githubusercontent.com/sabattle/CultBot/develop/data/images/CultBot.png')
        .addField(
      ...
      embed
        .setTitle('CultBot\'s Alias Types')
        .setDescription(stripIndent`
          **Prefix:** \`${prefix}\`
          **More Information:** \`${prefix}aliases [command type]\`
        `)
        .setImage('https://raw.githubusercontent.com/sabattle/CultBot/develop/data/images/Calypso_Title.png')
        .setFooter(message.member.displayName, message.author.displayAvatarURL({ dynamic: true }))
        .setTimestamp()
        .setColor(message.guild.me.displayHexColor);
