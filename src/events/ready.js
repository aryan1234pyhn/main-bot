module.exports = async (client) => {
  ...
  const activities = [
    ...
    { name: 'CultBot v.01', type: 'LISTENING' }
  ];
  ...
  for (const guild of leftGuilds) {
    ...
    client.logger.info(`CultBot has left ${guild.guild_name}`);
  }

  client.logger.info('CultBot is now online');
  client.logger.info(`CultBot is running on ${client.guilds.cache.size} server(s)`);
};
